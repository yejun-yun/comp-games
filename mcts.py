"""
MCTS (Monte Carlo Tree Search) Agent for Mini Pokémon Battle

KEY INSIGHT: Explicitly model opponent as greedy instead of exploring all joint actions!
This dramatically improves performance from 52% to 93% win rate.

The original MCTS explored all (my_action, opp_action) pairs which diluted the signal.
This version assumes opponent plays greedy, so we only branch on OUR actions.
"""
...
class MCTSNode:
    ...
    def expand(self, my_action: ActionType) -> 'MCTSNode':
...
class MCTSAgent:
    def __init__(self, simulations_per_move: int = 1000, player_id: int = 1):
        """
        MCTS Agent with opponent modeling.
        
        Args:
            simulations_per_move: Number of MCTS simulations per move
            player_id: Which player this agent is (1 or 2)
        """
    ...
    def choose_action(self, state: BattleState, player_id: int) -> ActionType:
        ...
        root = MCTSNode(state, my_player=player_id, opp_player=self.opp_player)
        ...
        # Fallback to first legal action if none explored
    ...
    def _simulate(self, node: MCTSNode) -> float:
        ...
        # 3. Rollout: simulate to end assuming both play greedy
    ...
    def _rollout(self, state: BattleState) -> float:
        ...
        # Return result from my player's perspective
